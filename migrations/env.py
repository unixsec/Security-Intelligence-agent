"""Alembic migration environment — async MySQL support."""
...
async def run_async_migrations() -> None:
    ...
    Reads the live DB URL from SIA settings (environment-driven) instead of
    the placeholder value in alembic.ini so no credentials are committed.
    """
    from sia.config import get_settings

    section = config.get_section(config.config_ini_section, {}) or {}
    settings = get_settings()
    section["sqlalchemy.url"] = settings.db.async_url
